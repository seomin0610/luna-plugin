...
const pickClosestToCenter = (lines: HTMLElement[], container: HTMLElement | null) => {
	...
	if (best) return best;
	// Fallback: no visible lines, just pick nearest by distance without visibility filter
	for (const line of lines) {
		const rect = line.getBoundingClientRect();
		const distance = Math.abs(rect.top + rect.height / 2 - centerY);
...
const findCurrentLine = () => {
	...
	// Find nested current markers and climb up
	...
	// Heuristic 1: pick the highest-scoring line
	let best: HTMLElement | null = null;
	let bestScore = 0;
	...
	// Heuristic 2: pick the line closest to the container center
